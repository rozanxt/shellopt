function [csm, lnk] = setup_c1c_scalar(sim, mdl)
    csm = zeros(sim.ndf);
    lnk = 1:sim.ndf;
...
%         csm(sim.dfs{pcb}, sim.dfs{pcb}) = csm(sim.dfs{pcb}, sim.dfs{pcb})+cbb;
        ...
        csm(csa, csa) = csm(csa, csa) + [1, -1; -1, 1];
        csm(csb, csb) = csm(csb, csb) + [1, -1; -1, 1];
        csm(csa, csb) = csm(csa, csb) + [-1, 1; 1, -1];
        csm(csb, csa) = csm(csb, csa) + [-1, 1; 1, -1];
