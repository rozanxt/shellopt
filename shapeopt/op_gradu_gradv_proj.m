...
function varargout = op_gradu_gradv_proj (spu, spv, msh, coeff)
  ...
  sgu = reshape (permute (gradu, [1, 3, 5, 4, 2]), 3, []);
  sgv = reshape (permute (gradv, [1, 3, 5, 4, 2]), 3, []);
  
  nmu = reshape (repmat(msh.normal, 1, 1, 1, spu.nsh_max * size(gradu, 2)), 3, []);
  nmv = reshape (repmat(msh.normal, 1, 1, 1, spv.nsh_max * size(gradv, 2)), 3, []);
  
  sgu = sgu - dot (nmu, sgu) .* nmu;
  sgv = sgv - dot (nmv, sgv) .* nmv;
  
  gradu = permute(reshape (sgu, spu.ncomp, msh.nqn, msh.nel, spu.nsh_max, []), [1, 5, 2, 4, 3]);
  gradv = permute(reshape (sgv, spv.ncomp, msh.nqn, msh.nel, spv.nsh_max, []), [1, 5, 2, 4, 3]);
