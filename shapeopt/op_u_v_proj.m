...
function varargout = op_u_v_proj (spu, spv, msh, coeff)
  ...
  sfu = reshape (permute (shpu, [1, 2, 4, 3]), 3, []);
  sfv = reshape (permute (shpv, [1, 2, 4, 3]), 3, []);
  
  nmu = reshape (repmat(msh.normal, 1, 1, 1, spu.nsh_max), 3, []);
  nmv = reshape (repmat(msh.normal, 1, 1, 1, spv.nsh_max), 3, []);
  
  sfu = sfu - dot (nmu, sfu) .* nmu;
  sfv = sfv - dot (nmv, sfv) .* nmv;
  
  shpu = permute(reshape (sfu, spu.ncomp, msh.nqn, msh.nel, spu.nsh_max), [1, 2, 4, 3]);
  shpv = permute(reshape (sfv, spv.ncomp, msh.nqn, msh.nel, spv.nsh_max), [1, 2, 4, 3]);
